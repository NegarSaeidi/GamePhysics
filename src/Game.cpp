...
bool Game::init(const char* title, const int x, const int y, const int width, const int height, const bool fullscreen)
{
	...
	if (SDL_Init(SDL_INIT_EVERYTHING) >= 0)
	{
		...
		if (m_pWindow != nullptr)
		{
			...
			SoundManager::Instance().load("../Assets/audio/start.ogg", "start", SOUND_MUSIC);
			SoundManager::Instance().load("../Assets/audio/space.mp3", "space", SOUND_MUSIC);
			SoundManager::Instance().load("../Assets/audio/throw.wav", "throw", SOUND_SFX);
			SoundManager::Instance().load("../Assets/audio/click.wav", "click", SOUND_SFX);
